...
def draw():
    ...
    if game_over:
        display_msg("GAME OVER!!","Please try again :)")
    elif game_complete:
        display_msg("HOORAY !! YOU WON. ","Good Job !!")
    else:
        for star in stars:
            star.draw()
...
def get_color_to_create(number_of_extra_stars):
    ...
    for i in range(0, number_of_extra_stars):
        random_clr = random.choice(COLORS)
        colors_to_create.append(random_clr)
...
def red_star_click():
    global stars, animations, current_level, game_complete
